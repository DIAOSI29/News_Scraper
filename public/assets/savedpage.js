window.onload = function() {
  var savedNewsContainer = $("#saved-news-container");

  function initPage() {
    $.get("/savedNews").then(function(data) {
      savedNewsContainer.empty();

      console.log(data);
      if (data && data.length) {
        renderSavedNews(data);
      } else {
        renderEmpty();
      }
    });
  ...
  function renderSavedNews(news) {
    ...
    for (var i = 0; i < news.length; i++) {
      newsList.push(createSavedNewsCard(news[i]));
    }
    savedNewsContainer.append(newsList);
  }
